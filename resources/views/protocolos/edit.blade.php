...
<div class="flash-message">
  ...
  <div class="form-row">
    <div id="div_desc" class="form-group col-md-4">
      ...
      <input type="text" id="desc"  name="desc" class="form-control form-control-sm " value="{{ count($errors) > 0 ? old('desc') : $protocolo->descripcion }}" placeholder="Nombre">
    </div>
  ...
  <div class="table-responsive">
    <table id ="myTable" class="table table-hover table-bordered table-sm">
      ...
      <tbody>
      @foreach($sintomas_actuales as $sint)
        <tr>
          <td>
            <input type="hidden" value="{{$sint->id}}" name="sint[]">
            <p>{{$sint->descripcion}}</p>
          </td>
          <td>
  ...
  <button id="ed_id" type="button" class="btn btn-mod">Editar</button>
...
// PARA VER SI EL VALOR DEL INPUT ES UN SÍNTOMA Y DEVOLVER LA POSICIÓN
  ...
  document.getElementById("ed_id").addEventListener("click", function(){
    var id = <?php echo $protocolo->id; ?>;
